...
\begin{itemize}
    \item Constructors \textit{build} values whereas Eliminators \textit{dismantle} values.
        \begin{itemize}
            ...
            \item \lstinline{(lambda (s) s)} builds a \lstinline{(-> Atom Atom)}; application is the eliminator for functions
        \end{itemize}
    \item The application eliminator for functions works by pure substitution. Hence, if \lstinline{f} is a \lstinline{(-> X Y)} then \lstinline{(lambda (y) (f y))} is the same \lstinline{(-> X Y)} as \lstinline{f} \textit{as long as \lstinline{y} does not appear in \lstinline{f}}. \textcolor{red}{Is this last condition really necessary in Pie, or have the authors introduced it just to make teaching the semantics of function application simpler?}
...
It is clear what \lstinline{apply} is \textit{supposed} to do: take a function and some collection of arguments, and apply the function to those arguments. It is also clear that this function can easily raise an error, but that's par for the course in Python. However, in Pie, we can actually give a reasonable type for \lstinline{apply}: \[
    \lstinline{(Pi ((a U) (k Nat)) (-> (NaryOp k a) (Vec a k) a))}
...
For example, suppose we have a function \[
    \lstinline{(claim double (Pi ((a U) (k Nat)) (-> (Vec a k) (Vec a (+ k k)))))}
\]
and a function \[
    \lstinline{(claim un-interleave (Pi ((a U) (k Nat)) (-> (Vec a (* 2 k)) (Pair (Vec a k) (Vec a k)))))}
\]
...
\begin{align*}
    ...
    \lstinline{mot} &\quad \text{a } \lstinline{(-> Nat U)} \\
    \lstinline{base} &\quad \text{a } \lstinline{(mot zero)} \\
    \lstinline{step} &\quad \text{a } \lstinline{(Pi ((k Nat)) (-> (mot k) (mot (add1 k))))}
\end{align*}
...
\\ \\
\lstinline{cong} is an eliminator for \lstinline{=}. Given a \lstinline{target} of type \lstinline{(= X from to)}, and a function \lstinline{f} of type \lstinline{(-> X Y)}, \lstinline{(cong target f)} returns a value of type \lstinline{(= Y (f from) (f to))}. \lstinline{cong} represents the well-definedness of functions; that is, if \(x = y\) then \(f(x) = f(y)\). \lstinline{(cong (same x) f)} computes as \lstinline{(same (f x))}.
...
If \begin{align*}
    ...
    \lstinline{mot} &\text{\quad is a } \lstinline{(-> (List E) U)} \\
    \lstinline{base} &\text{\quad is a } \lstinline{(mot nil)} \\
    \lstinline{step} &\text{\quad is a } \lstinline{(Pi ((e E) (es (List E))) (-> (mot es) (mot (:: e es))))}
\end{align*}
...
\subsection{Summary}
This chapter introduces the \lstinline{ind-Vec} eliminator, and uses it to define a version of \lstinline{append} which concatenates two values of a \lstinline{Vec} type and to rule out ``foolish" definitions of \lstinline{list->vec} from the preceding chapter.
...
If \begin{align*}
    ...
    \lstinline{mot} &\text{\quad is a } \lstinline{(Pi ((k Nat)) (-> (Vec E k) U))} \\
    \lstinline{base} &\text{\quad is a } \lstinline{(mot zero nil)} \\
    \lstinline{step} &\text{\quad is a } \lstinline{(Pi ((k Nat) (h E) (t (Vec E k))) (-> (mot k t) (mot (add1 k) (vec:: h t))))}
\end{align*}
...
\subsection{The \lstinline{Either} type}
For \lstinline{A}, \lstinline{B} types, the expression \lstinline{(Either A B)} is a type which represents ``\lstinline{A} or \lstinline{B}". A value of type \lstinline{(Either A B)} has one of two possible forms:
\begin{enumerate}
    \item \lstinline{(left x)}\phantom{t} for \lstinline{x} a value of type \lstinline{A}
    \item \lstinline{(right x)} for \lstinline{x} a value of type \lstinline{B}
\end{enumerate}
To eliminate a value of type \lstinline{(Either A B)}, we use \lstinline{ind-Either}, which takes four arguments,
\begin{align*}
    \lstinline{target} &\quad\text{a \lstinline{(Either A B)}} \\\
    \lstinline{motive} &\quad\text{a \lstinline{(-> (Either A B) U)}} \\
    \lstinline{on-left} &\quad\text{a \lstinline{(Pi ((l A)) (motive (left l)))}} \\
    \lstinline{on-right} &\quad\text{a \lstinline{(Pi ((r B)) (motive (right l)))}} 
\end{align*}
Given these four arguments, the expression \lstinline{(ind-Either target motive on-left on-right)} has type \lstinline{(motive target)} and has value \lstinline{(on-left l)} when \lstinline{target} is \lstinline{(left l)} and \lstinline{(on-right r)} when \lstinline{target} is \lstinline{(right r)}.
\subsection{Variable types with \lstinline{ind-Either}?}
\textcolor{red}{Is it possible to use the \lstinline{ind-Either} type to construct an expression whose type can be completely different depending on the \lstinline{target}? I wouldn't think so, as the motive argument must produce a \lstinline{U}, but maybe there is a way around it?}
\subsection{Generalizing \lstinline{even-or-odd}}
In this chapter, the authors use an interesting technique to prove that every number is either even or odd. They use induction to alternate between proofs of ``\(i\) is even" and ``\(i\) is odd" until they eventually have a proof of the parity of \(n\). \textcolor{red}{Can we generalize this proof technique to prove a statement like ``every number is either 0, 1, or 2 modulo 3" or even ``every number is either 0, 1, 2, \(\dots, m-1\) modulo \(m\)"?}
\subsection{Nested \lstinline{Either}s}
The above question leads to yet another: \textcolor{red}{how do we represent the disjunction of arbitrarily many types? How do we eliminate such a type?}
\newpage
\begin{appendix}
