...
\subsubsection{*distributes-over+ ((right) distributivity of multiplication over addition \((a + b)c = ac + bc\))} \label{code:*distributes-over+}
\begin{minted}{scheme}
...
(claim *distributes-over+
  (Pi ((a Nat) (b Nat) (c Nat))
    (= Nat (* (+ a b) c) (+ (* a c) (* b c)))))
(define *distributes-over+
  (lambda (a b c)
    (ind-Nat
      c
