...
(define transpose
  (lambda (m n a)
    (ind-Nat
      ...
      (lambda (w) (repeatvec (Vec a 0) n vecnil))
      (lambda (k-1 almost-trans)
        (ind-Nat
          n
          (lambda (i) (-> (Vec (Vec a i) (add1 k-1)) (Vec (Vec a (add1 k-1)) i)))
          (lambda (w) vecnil)
          (lambda (n-1 almost)
            (lambda (w)
              (vec::
                (mapvec (Vec a (add1 n-1)) a (add1 k-1) (lambda (v) (head v)) w)
                (almost
                  (mapvec (Vec a (add1 n-1)) (Vec a n-1) (add1 k-1) (lambda (v) (tail v)) w))))))))))
